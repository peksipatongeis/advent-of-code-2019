...
const createSegment = (directive, startPoint = { x: 0, y: 0 }) => {
  const direction = directive.charAt(0);
  const distance = parseInt(directive.substr(1));
  ...
  switch (direction) {
    case "U":
      return [startPoint, { x: x + distance, y }];
    case "R":
      return [startPoint, { x, y: y + distance }];
    case "D":
      return [startPoint, { x: x - distance, y }];
    case "L":
      return [startPoint, { x, y: y - distance }];
...
// Little bit of help to calculate the intersection point from
// @link https://stackoverflow.com/a/14795484
const getIntersectionPoint = (segment1, segment2) => {
  ...
  if (s_numer < 0 === denomIsPositive) {
    return false;
  }

  const t_numer = s32_x * s02_y - s32_y * s02_x;

  if (t_numer < 0 === denomIsPositive) {
    return false;
  }

  if (
    s_numer > denom === denomIsPositive ||
    t_numer > denom === denomIsPositive
  ) {
    return false;
  }

  const t = t_numer / denom;
  const intersectionPoint = { x: p0.x + t * s10_x, y: p0.y + t * s10_y };

  return intersectionPoint;
};

const getClosestIntersection = input => {
  const wireDirectives = input.split("\n");
  const wires = wireDirectives.map(wire => {
    const directives = wire.split(",");

    return directives.reduce((acc, directive, index) => {
      const prevSegment = acc[index - 1] || [
        { x: 0, y: 0 },
        { x: 0, y: 0 }
      ];
      const startPoint = prevSegment[1];

      acc.push(createSegment(directive, startPoint));
  ...
  const intersections = wire1.reduce((acc, ws1) => {
    const intersectionPoints = wire2.reduce((acc, ws2) => {
      const intersectionPoint = getIntersectionPoint(ws1, ws2);
    ...
    return [...acc, ...intersectionPoints];
  }, []);

  const closestIntersection = intersections.reduce((acc, point) => {
    const { x, y } = point;
    const distance = Math.abs(x) + Math.abs(y);

    if (!acc) return distance;
...
console.log(
  `Manhattan distance for the closest intersection point is ${getClosestIntersection(
    input
  )}`
);
